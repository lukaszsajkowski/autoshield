# Step 2: Attack Catalog — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create `attacks.py` with 50 prompt injection attacks (10 categories × 5 each) that serve as the fixed benchmark for the AutoShield research loop.

**Architecture:** Single file at project root exporting `ATTACKS: dict[str, list[str]]`. No dependencies beyond Python stdlib. Validated by a test that checks structure and completeness.

**Tech Stack:** Python 3.12, pytest (dev dependency via uv)
...
| File | Responsibility |
|---|---|
| `attacks.py` | 50 attack prompts organized in 10 categories |
| `tests/test_attacks.py` | Structural validation: correct keys, counts, types, canary presence |

---

### Task 1: Add pytest as dev dependency
...
Add this section after the existing `dependencies` list:
...
- [ ] **Step 2: Run `uv sync` to install pytest**

Run: `cd /Users/lukasz/Workspace/autoresearch && uv sync`

Expected: Output includes "Resolved" and "Installed" lines mentioning pytest. No errors.

- [ ] **Step 3: Verify pytest works**

Run: `cd /Users/lukasz/Workspace/autoresearch && uv run pytest --version`

Expected: Output shows pytest version 8.x+.
...
git commit -m "build: add pytest as dev dependency"
```

---

### Task 2: Write structural tests for attacks catalog
...
- [ ] **Step 2: Write `tests/test_attacks.py`**
...
Expected: FAIL — `ModuleNotFoundError: No module named 'attacks'` (file doesn't exist yet).

- [ ] **Step 4: Commit**

```bash
cd /Users/lukasz/Workspace/autoresearch
git add tests/__init__.py tests/test_attacks.py
git commit -m "test: add structural tests for attack catalog"
```

---

### Task 3: Create `attacks.py` with all 50 attacks

**Files:**
- Create: `attacks.py`

- [ ] **Step 1: Create `attacks.py`**
...
- `test_attacks_has_all_categories` — PASSED
- `test_each_category_has_five_attacks` — PASSED
- `test_all_attacks_are_nonempty_strings` — PASSED
- `test_no_duplicate_attacks` — PASSED
- `test_canary_appears_in_at_least_one_attack` — PASSED
...
cd /Users/lukasz/Workspace/autoresearch
git add attacks.py
git commit -m "feat: add attack catalog with 50 attacks across 10 categories"
```
